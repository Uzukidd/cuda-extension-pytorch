import torch
import numpy as np
from cudaext.ops.Rotated_IoU.box_intersection_2d import oriented_box_intersection_2d
from cudaext.ops.Rotated_IoU.min_enclosing_box import smallest_bounding_box
...
def box2corners_th(box: torch.Tensor) -> torch.Tensor:
    ...
    x4 = torch.FloatTensor(
        [0.5, -0.5, -0.5, 0.5]).unsqueeze(0).unsqueeze(0).to(box.device)  # (1,1,4)
    x4 = x4 * w     # (B, N, 4)
    y4 = torch.FloatTensor([0.5, 0.5, -0.5, -0.5]
                           ).unsqueeze(0).unsqueeze(0).to(box.device)
    y4 = y4 * h     # (B, N, 4)
...
def cal_iou_3d(box3d1: torch.Tensor, box3d2: torch.Tensor, verbose=False):
    ...
    if verbose:
        z_range = (torch.max(zmax1, zmax2) -
                   torch.min(zmin1, zmin2)).clamp_min(0.)
...
def cal_giou_3d(box3d1: torch.Tensor, box3d2: torch.Tensor, enclosing_type: str = "smallest"):
    ...
    """
    iou3d, corners1, corners2, z_range, u3d = cal_iou_3d(
        box3d1, box3d2, verbose=True)
    w, h = enclosing_box(corners1, corners2, enclosing_type)
    v_c = z_range * w * h
...
def enclosing_box_pca(corners1: torch.Tensor, corners2: torch.Tensor):
    ...
    v2 = v2.unsqueeze(1)
    # (B*N, 8), first principle component
    p1 = torch.sum(c * v1, dim=-1)
    # (B*N, 8), second principle component
    p2 = torch.sum(c * v2, dim=-1)
    # (B*N, ),  width of rotated enclosing box
    w = p1.max(dim=-1)[0] - p1.min(dim=-1)[0]
    # (B*N, ),  height of rotated enclosing box
...
def eigenvector_22(x: torch.Tensor):
    ...
    v1 = (a - b - delta) / 2. / c
    v1 = torch.stack([v1, torch.ones_like(
        v1, dtype=torch.double, device=v1.device)], dim=-1)    # (..., 2)
    v2 = (a - b + delta) / 2. / c
    v2 = torch.stack([v2, torch.ones_like(
        v2, dtype=torch.double, device=v2.device)], dim=-1)    # (..., 2)
...
def assign_target_3d(box3d_roi: torch.Tensor, box3d_gt: torch.Tensor):
    """calculated 3d IoU and assign target. assume the 3d bounding boxes are only rotated around z axis

    Args:
        box3d_roi (torch.Tensor): (N, 3+3+1),  (x,y,z,w,h,l,alpha)
        box3d_gt (torch.Tensor): (M, 3+3+1),  (x,y,z,w,h,l,alpha)

    Returns:
        (torch.Tensor): (M, N) 3d IoU
    ...
    box3d_roi_extended = box3d_roi.view(N, 1, 7).expand(N, M, -1)
    box3d_gt_extended = box3d_gt.view(1, M, 7).expand(N, M, -1)
    iou_3d = cal_iou_3d(box3d_roi_extended, box3d_gt_extended)
    
    iou_3d, assign_idx = iou_3d.max(dim=1)

    return iou_3d, assign_idx
