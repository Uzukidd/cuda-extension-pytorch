...
'''
import torch
from cudaext.ops.Rotated_IoU.cuda_op.cuda_ext import sort_v
EPSILON = 1e-8
...
def box1_in_box2(corners1: torch.Tensor, corners2: torch.Tensor):
    ...
    cond1 = (p_ab / norm_ab > - 1e-6) * \
        (p_ab / norm_ab < 1 + 1e-6)   # (B, N, 4)
    cond2 = (p_ad / norm_ad > - 1e-6) * \
        (p_ad / norm_ad < 1 + 1e-6)   # (B, N, 4)
    return cond1*cond2
...
def build_vertices(corners1: torch.Tensor, corners2: torch.Tensor,
    ...
    B = corners1.size()[0]
    N = corners1.size()[1]
    vertices = torch.cat([corners1, corners2, inters.view(
        [B, N, -1, 2])], dim=2)  # (B, N, 4+4+16, 2)
    mask = torch.cat([c1_in_2, c2_in_1, mask_inter.view(
        [B, N, -1])], dim=2)  # Bool (B, N, 4+4+16)
