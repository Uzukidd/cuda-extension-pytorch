...
def points_in_boxes_cpu(points, boxes):
    ...
    point_indices = points.new_zeros(
        (boxes.shape[0], points.shape[0]), dtype=torch.int)
...
def points_in_boxes_gpu(points, boxes):
    ...
    box_idxs_of_pts = points.new_zeros(
        (batch_size, num_points), dtype=torch.int).fill_(-1)
...
class RoIAwarePool3dFunction(Function):
    ...
    def forward(ctx, rois, pts, pts_feature, out_size, max_pts_each_voxel, pool_method):
        ...
        roiaware_pool3d_cuda.forward(
            rois, pts, pts_feature, argmax, pts_idx_of_voxels, pooled_features, pool_method)

        ctx.roiaware_pool3d_for_backward = (
            pts_idx_of_voxels, argmax, pool_method, num_pts, num_channels)
